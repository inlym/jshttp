 ...
 */
export function warnIfHeaderNameForbidden(name: string): void {
  /** 禁止修改的消息首部 */
...
export function xhrAdapter(request: HttpRequest): Observable<HttpResponse> {
  ...
  return new Observable((observer: Observer<HttpResponse>) => {
    ...
    request.headers.forEach((name: string, value: string) => {
      warnIfHeaderNameForbidden(name)
